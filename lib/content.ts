...
export function getItems(type: ContentType): ContentItem[] {
  ...
  return fs.readdirSync(directory).filter((file) => file.endsWith('.mdx')).map((file) => {
    ...
    return { ...parsed.data, date: formatDate(parsed.data.date), slug: file.replace(/\.mdx$/, ''), type, tags: parsed.data.tags ?? [], readingTime: readingTime(parsed.content).text } as ContentItem;
...
export function getItem(type: ContentType, slug: string) {
  ...
  return { ...parsed.data, date: formatDate(parsed.data.date), slug, type, content: parsed.content, readingTime: readingTime(parsed.content).text, tags: parsed.data.tags ?? [] } as ContentItem & { content: string; readingTime: string };
}
